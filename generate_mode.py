...
class LLMClient: #this class is responsible for interacting with the Google GenAI API to fetch questions based on a given topic.
    def __init__(self):
    ...
    def fetch_questions(self, topic: str, existing_questions: Optional[List[str]] = None) -> List[QuestionSchema]: 
        """Topic must be a string describing the subject area for the questions."""
        ...
        """Calls the LLM and returns a list of parsed Question objects --> List[QuestionSchema]"""
        
        # We build the exclusion text if we have existing questions to avoid repeats
        exclusion_text = ""
        if existing_questions:
            recent_qs = "\n".join(existing_questions[-15:]) # Here we could improve the code by using a random sample of existing questions instead of just the last 15, to provide a more diverse context to the LLM. 
        ...
        try:
            ...
            response = self.client.models.generate_content(
                ...
                response_mime_type='application/json', #we specify that we want the response in JSON format, which allows us to parse it directly into our QuestionList model.
                response_schema=QuestionList,
                temperature=0.9 #temperature is a parameter that controls the creativity of the LLM's responses. A higher value like 0.9 encourages more diverse and creative outputs.
                ) 
            )
...
class QuestionGenerator: #this is the main class that manages the question generation process, including interacting with the LLM and saving questions to JSON files.
    def __init__(self):
        self.llm = LLMClient()
    ...
    def run(self):
        ...
        for q in raw_questions:
            ...
            if action in ['a', 'e']: 
                """If "r", it simply skips the code below and goes to the next question, effectively rejecting it."""
                if action == 'e':
